"""
Example Flask web application using the HAR parser service abstraction.
Demonstrates how the web app can switch between different parser implementations
without code changes.
"""

import os
import tempfile
import json
import csv
import io
from datetime import datetime
from pathlib import Path

from flask import (
    Flask,
    request,
    jsonify,
    render_template,
    flash,
    redirect,
    url_for,
    make_response,
    session,
)
from werkzeug.utils import secure_filename

# Import the service layer
from services.config import get_config

app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "dev-key-change-in-production")
app.config["MAX_CONTENT_LENGTH"] = 16 * 1024 * 1024  # 16MB max file size


# Store last parse result in session for downloads
def store_result(result):
    """Store parse result in session for downloads."""
    session["last_result"] = {
        "success": result.success,
        "events": result.events,
        "event_count": result.event_count,
        "parse_time_ms": result.parse_time_ms,
        "service_used": result.service_used,
        "timestamp": datetime.now().isoformat(),
    }


def get_stored_result():
    """Get stored parse result from session."""
    return session.get("last_result")


@app.route("/")
def index():
    """Upload form page."""
    return render_template("index.html")


@app.route("/upload", methods=["POST"])
def upload_har():
    """Handle HAR file upload and parsing."""
    if "har_file" not in request.files:
        flash("No file selected", "error")
        return redirect(url_for("index"))

    file = request.files["har_file"]
    if file.filename == "":
        flash("No file selected", "error")
        return redirect(url_for("index"))

    if not file.filename.lower().endswith(".har"):
        flash("Please upload a .har file", "error")
        return redirect(url_for("index"))

    try:
        # Save uploaded file temporarily
        filename = secure_filename(file.filename)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".har") as tmp_file:
            file.save(tmp_file.name)

            # Get parser service and parse
            config = get_config()
            parser = config.create_parser()

            debug = request.form.get("debug") == "1"
            result = parser.parse_har_file(tmp_file.name, debug=debug)

            # Clean up temp file
            os.unlink(tmp_file.name)

            # Store result for downloads
            store_result(result)

            if result.success:
                flash(f"Successfully parsed {result.event_count} events", "success")
            else:
                flash(f"Parse failed: {result.error_message}", "error")

            return render_template(
                "results.html",
                result=result,
                debug_info=result.debug_info if debug else None,
            )

    except Exception as e:
        flash(f"Upload failed: {str(e)}", "error")
        return redirect(url_for("index"))


@app.route("/parse-example", methods=["POST"])
def parse_example():
    """Parse the example HAR file."""
    try:
        # Use the example HAR file
        example_path = Path(__file__).parent.parent / "parser" / "Example2.har"

        if not example_path.exists():
            flash("Example HAR file not found", "error")
            return redirect(url_for("index"))

        # Get parser service and parse
        config = get_config()
        parser = config.create_parser()

        result = parser.parse_har_file(str(example_path), debug=False)

        # Store result for downloads
        store_result(result)

        if result.success:
            flash(
                f"Successfully parsed {result.event_count} events from example file",
                "success",
            )
        else:
            flash(f"Parse failed: {result.error_message}", "error")

        return render_template("results.html", result=result)

    except Exception as e:
        flash(f"Example parse failed: {str(e)}", "error")
        return redirect(url_for("index"))


@app.route("/download/json")
def download_json():
    """Download last parse results as JSON."""
    result = get_stored_result()
    if not result or not result["success"]:
        flash("No results available for download", "error")
        return redirect(url_for("index"))

    # Create JSON response
    json_data = json.dumps(result["events"], indent=2)

    response = make_response(json_data)
    response.headers["Content-Type"] = "application/json"
    response.headers["Content-Disposition"] = (
        "attachment; filename=facebook_events.json"
    )

    return response


@app.route("/download/csv")
def download_csv():
    """Download last parse results as CSV."""
    result = get_stored_result()
    if not result or not result["success"]:
        flash("No results available for download", "error")
        return redirect(url_for("index"))

    # Create CSV content
    output = io.StringIO()
    writer = csv.writer(output)

    # Write header
    writer.writerow(["Name", "DateTime", "Location", "Details", "Link", "Event ID"])

    # Write events
    for event in result["events"]:
        writer.writerow(
            [
                event["name"],
                event["datetime"],
                event["location"] or "",
                event["details"] or "",
                event["link"],
                event["event_id"],
            ]
        )

    csv_data = output.getvalue()
    output.close()

    response = make_response(csv_data)
    response.headers["Content-Type"] = "text/csv"
    response.headers["Content-Disposition"] = "attachment; filename=facebook_events.csv"

    return response


@app.route("/api/docs")
def api_events():
    """API documentation page."""
    return render_template("api_docs.html")


# API Endpoints
@app.route("/api/events/parse", methods=["POST"])
def api_parse_events():
    """API endpoint for parsing HAR files."""
    if "har_file" not in request.files:
        return jsonify({"success": False, "error_message": "No file provided"}), 400

    file = request.files["har_file"]
    if file.filename == "":
        return (
            jsonify({"success": False, "error_message": "No file selected"}),
            400,
        )

    if not file.filename.lower().endswith(".har"):
        return (
            jsonify(
                {
                    "success": False,
                    "error_message": "Invalid file type. Only .har files are supported",
                }
            ),
            400,
        )

    try:
        # Save uploaded file temporarily
        with tempfile.NamedTemporaryFile(delete=False, suffix=".har") as tmp_file:
            file.save(tmp_file.name)

            # Get parser service and parse
            config = get_config()
            parser = config.create_parser()

            debug = request.form.get("debug", "").lower() in ("true", "1", "yes")
            result = parser.parse_har_file(tmp_file.name, debug=debug)

            # Clean up temp file
            os.unlink(tmp_file.name)

            # Return result as JSON
            return jsonify(
                {
                    "success": result.success,
                    "event_count": result.event_count,
                    "events": result.events,
                    "parse_time_ms": result.parse_time_ms,
                    "service_used": result.service_used,
                    "error_message": result.error_message,
                    "debug_info": result.debug_info if debug else None,
                }
            )

    except Exception as e:
        return jsonify({"success": False, "error_message": str(e)}), 500


@app.route("/api/health")
def api_health():
    """API health check endpoint."""
    return jsonify(
        {
            "status": "healthy",
            "service": "HAR Event Parser API",
            "version": "1.0.0",
            "timestamp": datetime.now().isoformat(),
        }
    )


@app.route("/api/info")
def api_info():
    """API service information endpoint."""
    config = get_config()
    parser = config.create_parser()

    return jsonify(
        {
            "service_type": config.service_type,
            "service_info": parser.get_service_info(),
            "supported_formats": [".har"],
            "max_file_size_mb": 16,
            "features": ["facebook_events", "json_output", "debug_mode"],
        }
    )


if __name__ == "__main__":
    # For development only
    app.run(debug=True, port=5000)

import os
import tempfile
import json
import csv
import io
from datetime import datetime
from pathlib import Path

from flask import (
    Flask,
    request,
    jsonify,
    render_template,
    flash,
    redirect,
    url_for,
    make_response,
    session,
)
from werkzeug.utils import secure_filename

# Import the service layer
from services.config import get_config

app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "dev-key-change-in-production")
app.config["MAX_CONTENT_LENGTH"] = 16 * 1024 * 1024  # 16MB max file size


# Store last parse result in session for downloads
def store_result(result):
    """Store parse result in session for downloads."""
    session["last_result"] = {
        "success": result.success,
        "events": result.events,
        "event_count": result.event_count,
        "parse_time_ms": result.parse_time_ms,
        "service_used": result.service_used,
        "timestamp": datetime.now().isoformat(),
    }


def get_stored_result():
    """Get stored parse result from session."""
    return session.get("last_result")


@app.route("/")
def index():
    """Upload form page."""
    return render_template("index.html")


@app.route("/upload", methods=["POST"])
def upload_har():
    """Handle HAR file upload and parsing."""
    if "har_file" not in request.files:
        flash("No file selected", "error")
        return redirect(url_for("index"))

    file = request.files["har_file"]
    if file.filename == "":
        flash("No file selected", "error")
        return redirect(url_for("index"))

    if not file.filename.lower().endswith(".har"):
        flash("Please upload a .har file", "error")
        return redirect(url_for("index"))

    try:
        # Save uploaded file temporarily
        filename = secure_filename(file.filename)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".har") as tmp_file:
            file.save(tmp_file.name)

            # Get parser service and parse
            config = get_config()
            parser = config.create_parser()

            debug = request.form.get("debug") == "1"
            result = parser.parse_har_file(tmp_file.name, debug=debug)

            # Clean up temp file
            os.unlink(tmp_file.name)

            # Store result for downloads
            store_result(result)

            if result.success:
                flash(f"Successfully parsed {result.event_count} events", "success")
            else:
                flash(f"Parse failed: {result.error_message}", "error")

            return render_template(
                "results.html",
                result=result,
                debug_info=result.debug_info if debug else None,
            )

    except Exception as e:
        flash(f"Upload failed: {str(e)}", "error")
        return redirect(url_for("index"))


@app.route("/parse-example", methods=["POST"])
def parse_example():
    """Parse the example HAR file."""
    try:
        # Use the example HAR file
        example_path = Path(__file__).parent.parent / "parser" / "Example2.har"

        if not example_path.exists():
            flash("Example HAR file not found", "error")
            return redirect(url_for("index"))

        # Get parser service and parse
        config = get_config()
        parser = config.create_parser()

        result = parser.parse_har_file(str(example_path), debug=False)

        # Store result for downloads
        store_result(result)

        if result.success:
            flash(
                f"Successfully parsed {result.event_count} events from example file",
                "success",
            )
        else:
            flash(f"Parse failed: {result.error_message}", "error")

        return render_template("results.html", result=result)

    except Exception as e:
        flash(f"Example parse failed: {str(e)}", "error")
        return redirect(url_for("index"))


@app.route("/download/json")
def download_json():
    """Download last parse results as JSON."""
    result = get_stored_result()
    if not result or not result["success"]:
        flash("No results available for download", "error")
        return redirect(url_for("index"))

    # Create JSON response
    json_data = json.dumps(result["events"], indent=2)

    response = make_response(json_data)
    response.headers["Content-Type"] = "application/json"
    response.headers["Content-Disposition"] = (
        "attachment; filename=facebook_events.json"
    )

    return response


@app.route("/download/csv")
def download_csv():
    """Download last parse results as CSV."""
    result = get_stored_result()
    if not result or not result["success"]:
        flash("No results available for download", "error")
        return redirect(url_for("index"))

    # Create CSV content
    output = io.StringIO()
    writer = csv.writer(output)

    # Write header
    writer.writerow(["Name", "DateTime", "Location", "Details", "Link", "Event ID"])

    # Write events
    for event in result["events"]:
        writer.writerow(
            [
                event["name"],
                event["datetime"],
                event["location"] or "",
                event["details"] or "",
                event["link"],
                event["event_id"],
            ]
        )

    csv_data = output.getvalue()
    output.close()

    response = make_response(csv_data)
    response.headers["Content-Type"] = "text/csv"
    response.headers["Content-Disposition"] = "attachment; filename=facebook_events.csv"

    return response


@app.route("/api/docs")
def api_events():
    """API documentation page."""
    return render_template("api_docs.html")


# API Endpoints
@app.route("/api/events/parse", methods=["POST"])
def api_parse_events():
    """API endpoint for parsing HAR files."""
    if "har_file" not in request.files:
        return jsonify({"success": False, "error_message": "No file provided"}), 400

    file = request.files["har_file"]
    if file.filename == "":
        return jsonify({"success": False, "error_message": "No file selected"}), 400

    if not file.filename.lower().endswith(".har"):
        return (
            jsonify(
                {
                    "success": False,
                    "error_message": "Invalid file type. Only .har files are supported",
                }
            ),
            400,
        )

    try:
        # Save uploaded file temporarily
        with tempfile.NamedTemporaryFile(delete=False, suffix=".har") as tmp_file:
            file.save(tmp_file.name)

            # Get parser service and parse
            config = get_config()
            parser = config.create_parser()

            debug = request.form.get("debug", "").lower() in ("true", "1", "yes")
            result = parser.parse_har_file(tmp_file.name, debug=debug)

            # Clean up temp file
            os.unlink(tmp_file.name)

            # Return result as JSON
            return jsonify(
                {
                    "success": result.success,
                    "event_count": result.event_count,
                    "events": result.events,
                    "parse_time_ms": result.parse_time_ms,
                    "service_used": result.service_used,
                    "error_message": result.error_message,
                    "debug_info": result.debug_info if debug else None,
                }
            )

    except Exception as e:
        return jsonify({"success": False, "error_message": str(e)}), 500


@app.route("/api/health")
def api_health():
    """API health check endpoint."""
    return jsonify(
        {
            "status": "healthy",
            "service": "HAR Event Parser API",
            "version": "1.0.0",
            "timestamp": datetime.now().isoformat(),
        }
    )


@app.route("/api/info")
def api_info():
    """API service information endpoint."""
    config = get_config()
    parser = config.create_parser()

    return jsonify(
        {
            "service_type": config.service_type,
            "service_info": parser.get_service_info(),
            "supported_formats": [".har"],
            "max_file_size_mb": 16,
            "features": ["facebook_events", "json_output", "debug_mode"],
        }
    )


if __name__ == "__main__":
    # For development only
    app.run(debug=True, port=5000)

from flask import Flask, request, jsonify, render_template, flash, redirect, url_for
from werkzeug.utils import secure_filename
import os
from pathlib import Path

# Import the service abstraction
from ..services import HarParserFactory
from ..services.config import get_config

app = Flask(__name__)
app.secret_key = "your-secret-key-here"
app.config["MAX_CONTENT_LENGTH"] = 50 * 1024 * 1024  # 50MB max file size

# Configure upload folder
UPLOAD_FOLDER = Path("uploads")
UPLOAD_FOLDER.mkdir(exist_ok=True)
app.config["UPLOAD_FOLDER"] = UPLOAD_FOLDER

# Initialize parser service using configuration
config = get_config()
parser_service = config.create_parser()


@app.route("/")
def index():
    """Main page with file upload form."""
    service_info = parser_service.get_service_info()
    return render_template("index.html", service_info=service_info)


@app.route("/upload", methods=["POST"])
def upload_har():
    """Handle HAR file upload and parsing."""
    if "har_file" not in request.files:
        flash("No file selected")
        return redirect(url_for("index"))

    file = request.files["har_file"]
    if file.filename == "":
        flash("No file selected")
        return redirect(url_for("index"))

    if file and file.filename.lower().endswith(".har"):
        # Save uploaded file
        filename = secure_filename(file.filename)
        file_path = app.config["UPLOAD_FOLDER"] / filename
        file.save(str(file_path))

        try:
            # Validate the HAR file
            if not parser_service.validate_har_file(file_path):
                flash("Invalid HAR file format")
                return redirect(url_for("index"))

            # Parse the HAR file using the service abstraction
            debug_mode = request.form.get("debug", False)
            result = parser_service.parse_har_file(file_path, debug=debug_mode)

            if result.success:
                # Store events in session or database
                # For this example, we'll just return them
                return render_template(
                    "results.html",
                    events=result.events,
                    event_count=result.event_count,
                    filename=filename,
                )
            else:
                flash(f"Parsing failed: {result.error_message}")
                return redirect(url_for("index"))

        except Exception as e:
            flash(f"Unexpected error: {str(e)}")
            return redirect(url_for("index"))
        finally:
            # Clean up uploaded file
            if file_path.exists():
                file_path.unlink()

    else:
        flash("Please upload a .har file")
        return redirect(url_for("index"))


@app.route("/api/parse", methods=["POST"])
def api_parse():
    """API endpoint for HAR parsing (for AJAX calls)."""
    if "har_file" not in request.files:
        return jsonify({"error": "No file provided"}), 400

    file = request.files["har_file"]
    if not file.filename.lower().endswith(".har"):
        return jsonify({"error": "File must be a .har file"}), 400

    # Save temporary file
    filename = secure_filename(file.filename)
    file_path = app.config["UPLOAD_FOLDER"] / f"temp_{filename}"
    file.save(str(file_path))

    try:
        # Parse using service abstraction
        debug_mode = request.form.get("debug", "false").lower() == "true"
        result = parser_service.parse_har_file(file_path, debug=debug_mode)

        if result.success:
            return jsonify(
                {
                    "success": True,
                    "events": result.events,
                    "event_count": result.event_count,
                }
            )
        else:
            return jsonify({"success": False, "error": result.error_message}), 400

    except Exception as e:
        return jsonify({"success": False, "error": f"Unexpected error: {str(e)}"}), 500
    finally:
        # Clean up
        if file_path.exists():
            file_path.unlink()


@app.route("/api/service-info")
def api_service_info():
    """Get information about the current parser service."""
    return jsonify(parser_service.get_service_info())


@app.route("/health")
def health():
    """Health check endpoint."""
    try:
        service_info = parser_service.get_service_info()
        return jsonify(
            {
                "status": "healthy",
                "service": service_info.get("name", "Unknown"),
                "type": service_info.get("type", "Unknown"),
            }
        )
    except Exception as e:
        return jsonify({"status": "unhealthy", "error": str(e)}), 500


if __name__ == "__main__":
    print(f"Starting web app with {parser_service.get_service_info()['name']}")
    app.run(debug=True, port=5000)
